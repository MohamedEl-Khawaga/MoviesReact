...
export default function Tv() {
  ...
  async function getTrendingMovies(pageNumper) {
    let { data } = await Axios.get(
      `
      https://api.themoviedb.org/3/discover/tv?api_key=eba8b9a7199efdcb0ca1f96879b83c44&language=en-US&sort_by=popularity.desc&page=${pageNumper}`
    );
  ...
  function addClass(){
    $('.pagination li').toggleClass('active').removeClass("notActive")
  }
  return (
    <>
      {( isLoding === 'true' ) ? (
        ...
        <div className="row justify-content-center ">
          ...
          >
            <ul className="pagination  ">
              {num.map((bage, i) => (
                <li key={i} className="page-item notActive ">
                  <a
                    
                    onClick= {() =>` ${getTrendingMovies(bage)} ${addClass()}`}
